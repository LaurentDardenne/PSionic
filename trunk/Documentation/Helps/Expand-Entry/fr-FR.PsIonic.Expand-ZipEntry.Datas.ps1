...
$Datas = @{
	...
	ExpandZipEntryDescription = 'Extrait des entrées d''une archive compressée au format Zip, les données extraites sont des données sérialisées permettant de reconstruire un objet.'
	ExpandZipEntrySetsDefault = ''
    ExpandZipEntryParametersAsHashTable= 'Renvoi les entrées lues dans un objet hashtable. Les noms des entrées à extraire seront utilisées en tant que nom de clé.'
	...
	ExpandZipEntryParametersStrict = 'Les errreurs déclenchent une exception au lieu d''être affichées.'
	ExpandZipEntryParametersXML = 'L''objet sérialisé est renvoyé au format XML.'
