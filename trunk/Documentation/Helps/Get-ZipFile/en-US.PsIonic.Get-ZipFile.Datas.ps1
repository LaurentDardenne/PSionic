...
"@
	...
	GetZipFileParametersZipErrorAction = 'Specify the error mode handling.'
...
Pay attention that for each returned object you must call the Close() method in order to properly release resources archive.
"@
...
This example reads a Zip file, adds .txt files fro ma folder to it, saves it and then releases resources archive.
...
This example opens an archive, delete all the .TXT files and adds all .LOG files from C:\Temp folder.
