...
$Datas = @{
	...
	AddZipEntryParametersDirectoryPath = 'La nouvelle entrée sera ajoutée dans un répertoire spécifique. Par défaut elle est ajoutée à la racine de l''arborescence contenu dans l''archive.'
...
"@
	AddZipEntryParametersPassthru = 'Une fois l''entrée aujoutée au catalogue de l''archive, elle est emise dans le pipeline.'
...
Ces commandes ajoutent une entrée dans l'archive C:\Temp\Test.zip. 
  ...
  -la quatrième enregistre l'archive sur le disque et 
  -la dernière libére les ressources systèmes.  
"@
...
Ces commandes ajoutent des entrées dans l'archive nommée C:\Temp\Test.zip. 
  ...
  -la troisème enregistre l'archive sur le disque et 
  -la dernière libére les ressources systèmes.
"@        
...
Ces commandes ajoutent, à partir d'une chaîne de caratères, une entrée nommée dans l'archive C:\Temp\Test.zip. 
  -La première instruction crée un objet archive à partir d'un nom de fichier, 
  -la seconde affecte un saut de ligne dans la variable automatique 'Output Field Separator', 
  -la troisième lit un fichier texte et récupère le résultat dans une chaîne de caractères,
  -la quatrième crée une entrée dont le contenu est renseigné à partir d'une chaîne de caractères, 
  -les  dernières enregistrent l'archive et libérent les ressources systèmes.
"@  
    AddZipEntryExamplesRemarks4 = 'TODO'
}
